...
function nlsfitworker(model::Function, y::Matrix{Float64},
  ...
  for j in 1:n
    ...
    results = levenberg_marquardt(f, g, p0; tolX=1e-4, tolG=1e-6,
                                        lambda=10.0)
...
function nlsfit(f::Function, y::Matrix{Float64}, idxs::Vector{Int64},
  ...
  if nw > 1
    ...
    println("fitting $nt x $idxperworker points on each of $nw workers")
    workerids = workers()
    for w in 1:nw
    ...
    println("running $nt x $nidxs points on one CPU core")
    params[:,idxs], resids[:,idxs] = nlsfitworker(f, y[:,idxs], x, p0)
  end
  t = toq()
  vps = nidxs / t
  @printf "processed %d vectors in %.1f s (%.1f vec/s)\n" nidxs t vps
  (params, resids, dof)
end
