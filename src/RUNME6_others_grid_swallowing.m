...
data = double(squeeze(raread('../data/swallowing/raw_012.ra')));
nchan = size(data,1);
nro = size(data,2);
npe = size(data,3);
npe_per_slice = 204;
...
rawrite(img_irt, 'output/img_sw_irt.ra');
...
img_gn = sqrt(img_gn);
rawrite(img_gn, 'output/img_sw_gn.ra');
...
rawrite(img_bart,'output/img_sw_bart.ra');
