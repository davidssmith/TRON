...
data = double(squeeze(raread('../data/swallowing.ra')));
nchan = size(data,1);
nro = size(data,2);
npe = size(data,3);
npe_per_slice = nro/2;
...
if exist(outfile) == 0
  ...
  writetable(T,'figs/sw_timings.csv');
...
img_tron = squeeze(raread('output/img_sw_tron.ra'));
img_irt = squeeze(raread('output/img_sw_irt.ra'));
img_gn = squeeze(raread('output/img_sw_gn.ra'));
img_bart = squeeze(raread('output/img_sw_bart.ra'));
